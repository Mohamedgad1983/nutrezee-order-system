...
| 8 | Visible bilingual and operational acceptance | **PROVEN** | Fixture Chromium Playwright passed 2/2. Protected live Playwright passed the applicable bilingual totals test. The follow-up English-default release sets the initial document, login, and display to English/LTR, migrates past the old Arabic-default preference key once, and retains the Arabic/RTL toggle as an explicit persistent operator choice. Production browser acceptance covers login, six populated section cards, both directions, logout, and 390×800 layout with no horizontal overflow. | None. |
