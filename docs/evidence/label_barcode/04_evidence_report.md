...
| R4 | ~~`/collection/*` is not yet reachable on staging.~~ | ✅ **RESOLVED 2026-07-27.** Deployed: migrations 0020 + 0027 applied (scoped, on owner decision), api + admin images rebuilt from the branch and recreated. `/nz/collection/manifest` now returns `401 {"error_code":"no_session"}` — API JSON, not SPA HTML. See `06_staging_deployment.md`. |
| R4b | Staging runs the **unmerged branch**, and migrations 0024–0026 remain pending there. | Deliberate. A future plain `migrate.mjs` run will apply them, including 0025's RBAC change. Recorded as a hand-off. |
