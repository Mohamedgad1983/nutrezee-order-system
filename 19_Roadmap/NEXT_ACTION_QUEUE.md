...
**Last updated:** 2026-08-08 · **Frontier:** **WP-KDS-02 is DONE and production-active under A35; no KDS blocker remains. WP-OPS-03 and WP-OPS-02 remain code/CI-complete but BLOCKED AT RELEASE under A22/A21.** Staging still lacks migrations `0025/0026`, the two protected least-privilege Fleetbase service identities/tokens, and a dedicated named Logistics Manager account; PR #43 remains intentionally unmerged until those release inputs exist. **WP-OPS-01's July 20 operational run is complete**, while the separate A23/A24 Partner snapshot stays read-only/non-authoritative at 01:00 Kuwait and the unattended dispatch timer remains disabled. WP-LBL-A27/A28/A29 is deployed inside Fleet-Ops with permanent barcodes and exact read-only Partner v2 nutrition sourcing; 552 complete orders are renderable and 187 incomplete orders block explicitly, while complete daily Fleetbase assignment and the physical 100 × 70 mm print/camera pilot remain operational dependencies. The legacy-replacement engineering frontier is exhausted and sponsor/workshop-gated. · **Goal:** replace the legacy daily order operation (not MVP theory) — see `Legacy_Core_Gap_To_Cutover.md`. Staging remains seeded for UAT with `cutover_catalog=false`.
