...
The standalone Kitchen Display is production-active at `https://kds.13-140-159-201.sslip.io` on release `3743aea`. The user supplied the dedicated production Partner credential and explicitly directed autonomous production activation. It is held only in the protected server-side secret file; it was never committed, printed, returned to the browser, or found in the container logs. A random initial display credential was generated server-side, its scrypt hash was installed, and its plaintext handoff remains root-only at `/root/nutrezee-kds-display-initial-password` for operator retrieval and rotation. User-directed A19 makes English/LTR the initial login/display language while retaining an explicit persistent Arabic/RTL toggle.
