...
| ASM-051 | Standalone KDS Release-1 operating values | Until the deferred kitchen questions are answered, use one independent shared-display credential, configurable kitchen allow-list with `main` as the development/staging default, 60-second browser refresh, 15-second in-process source cache, and totals only by exact upstream section + `meal_id` + `portion_size`. Missing routes remain visibly `unrouted`; no status writes or inferred routing. | High | Yes | WP-KDS-01 | Kitchen operations review; replace values through KDS environment configuration. |
