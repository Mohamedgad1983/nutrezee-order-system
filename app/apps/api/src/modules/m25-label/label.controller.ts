...
// m25-label — exact legacy label, permanent customer barcode, daily box collection (A27).
//
...
export class LabelController {
  ...
  async manifest(@Req() req: Request, @Query('date') date?: string) {
    const ctx = await this.ctx(req);
    await requirePermission(this.access, ctx, 'collection.manifest.read');
    const grants = await this.access.visibilityGrants(ctx.roles);
    return this.wrap(async () => {
      const m = await this.collection.manifest(ctx, date);
      return {
        ...m,
        entries: m.entries.map((e) => {
          const { data, masked } = maskFields(
            e as unknown as Record<string, unknown>, { customer_name: 'pii' }, grants,
          );
          return { ...data, masked };
        }),
      };
  ...
  async scan(
    ...
    const ctx = await this.ctx(req);
    await requirePermission(this.access, ctx, 'collection.scan');
    if (!body?.barcode?.trim()) throw new BadRequestException({ error_code: 'validation_failed', field: 'barcode' });
    const grants = await this.access.visibilityGrants(ctx.roles);
    return this.wrap(async () => {
      const result = await this.collection.scan(ctx, {
        barcode: body.barcode, delivery_date: body.delivery_date, device_ref: body.device_ref,
      }, idempotencyKey);
      const { data, masked } = maskFields(
        result as unknown as Record<string, unknown>, { customer_name: 'pii' }, grants,
      );
      return { ...data, masked };
    });
  }
