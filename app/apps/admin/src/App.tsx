...
export function App(): React.JSX.Element {
  if (window.location.pathname === '/kitchen') return <KitchenBoard />;
  return (
    <main className="shell">
      <nav className="topbar">
        <strong>Nutrezee</strong>
        <a href="/kitchen">Kitchen</a>
      </nav>
      <section className="empty">
        <h1>Nutrezee Admin</h1>
        <a className="primaryLink" href="/kitchen">Open Kitchen Board</a>
      </section>
    </main>
  );
}

function KitchenBoard(): React.JSX.Element {
  ...
  async function transition(ticket: KitchenTicket, to: KitchenTicket['status']) {
    setBusy(true);
    setError(null);
    try {
  ...
  async function pack(dayId: string) {
    setBusy(true);
    setError(null);
    try {
